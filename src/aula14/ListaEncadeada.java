...
public class ListaEncadeada {
    ...
    public Object retirarUltimo() {
        ...
        Celula auxiliar = this.cabeca; //Vai referenciar a penúltima célula

        while(!(auxiliar.proximo == this.ultimo)) {
            auxiliar = auxiliar.proximo;
        }

        this.ultimo = auxiliar; //A penúltima célula se torna a última
        this.ultimo.proximo = null; //Indica o fim da lista

        return item; //Retorna o último
    }
    ...
    public boolean concatenar(ListaEncadeada l2) { //Adiciona os elementos de outra lista ao final desta
        if(l2.vazia()) return false; //Erro

        this.ultimo.proximo = l2.cabeca.proximo; //O ultimo desta lista passa a apontar para a primeira célula da outra
        this.ultimo = l2.ultimo; //O ultimo de l2 torna-se o ultimo da lista concatenada
